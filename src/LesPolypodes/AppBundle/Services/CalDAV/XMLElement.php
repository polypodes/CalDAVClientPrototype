...
class XMLElement
{
  ...
  */
    function __construct($tagname, $content = false, $attributes = false, $xmlns = null)
    {
        ...
        if (isset($xmlns)) {
            ...
            } elseif (isset($this->attributes['xmlns'])) {
  ...
  */
    function GetElements($tag = null, $recursive = false)
    {
        ...
        } elseif (empty($tag) || (isset($v->content->tagname) && $v->content->GetNSTag() == $tag)) {
  ...
  */
    function RenderContent($indent = 0, $nslist = null, $force_xmlns = false)
    {
      ...
      */
            ...
            } elseif (defined('ENT_XML1') && defined('ENT_DISALLOWED')) {
  ...
  */
    function Render($indent = 0, $xmldef = "", $nslist = null, $force_xmlns = false)
    {
      ...
      */
        ...
        //          if ( $force_xmlns ) printf( "1: %s: %s\n", $this->tagname, $this->xmlns );
                    ...
                    if (isset($nslist[$v]) && $nslist[$v] == $prefix) {
                        continue; // No need to include in list as it's in a wrapping element
                    }            $nslist[$v] = $prefix;
                    if (!isset($this->xmlns)) {
                        $this->xmlns = $v;
                    }
    ...
    //      if ( $force_xmlns ) printf( "2: %s: %s\n", $this->tagname, $this->xmlns );
        ...
        } elseif (isset($this->xmlns) && !isset($nslist[$this->xmlns]) && gettype($this->attributes) == 'array' && !isset($this->attributes[$this->xmlns])) {
    //      if ( $force_xmlns ) printf( "3: %s: %s\n", $this->tagname, $this->xmlns );
            $attr .= sprintf(' xmlns="%s"', $this->xmlns);
        } elseif ($force_xmlns && isset($this->xmlns) && ! $xmlns_done) {
    //      printf( "4: %s: %s\n", $this->tagname, $this->xmlns );
            $attr .= sprintf(' xmlns="%s"', $this->xmlns);
        }

...
if (!function_exists('dbg_error_log')) {
    ...
    function dbg_error_log()
    {
        ...
        } elseif ($component == "ERROR") {
            $type = "***";
        } elseif (isset($c->dbg["ALL"])) {
            $type = "ALL";
        } elseif (!isset($c->dbg[strtolower($component)])) {
            return;
        }
...
EOERRMSG;
        ...
        } elseif (preg_match('/^\*(.+)\*.+$/', $we_have, $regs)) {
...
function olson_from_tzstring($tzstring)
{
    ...
    if (function_exists('timezone_identifiers_list') && in_array($tzstring, timezone_identifiers_list())) {
        return $tzstring;
    }
...
if (!function_exists("deprecated")) {
    function deprecated($method)
    {
        ...
        if (isset($c->dbg['ALL']) || isset($c->dbg['deprecated'])) {
            ...
            array_shift($stack);
            if (preg_match('{/inc/iCalendar.php$}', $stack[0]['file']) && $stack[0]['line'] > __LINE__) {
                return;
            }

...
class CalDAVClient
{
    ...
    public function __construct($base_url, $user, $pass, $options = array())
    {
        $this->user = $user;
        $this->pass = $pass;
        $this->headers = array();
     ...
     *
     */
    public function isValidCalDAVServer()
    {
        // Clean headers
        $this->headers = array();
...
// die(var_dump($dav_options));
     ...
     */
    public function DoOptionsRequestAndGetDAVHeader($url = null)
    {
    ...
    public function DoRequest($url = null)
    {
        ...
        if (!isset($this->headers['content-type'])) {
            $this->headers['content-type'] = "Content-type: text/plain";
        }
    ...
    {
        $lines = preg_split('/[\r\n]+/', $headers);
        $this->httpResultCode = preg_replace(
            '/^[\S]+ (\d+).+$/',
            '\1',
            $lines[0]
        );
     ...
     */
    public function DoOptionsRequest($url = null)
    {
        $this->requestMethod = "OPTIONS";
        $this->body = "";
        $headers = $this->DoRequest($url);
     ...
     * @return string The content of the response from the server
     */
    public function DoPUTRequest($url, $icalendar, $etag = null)
    {
        $this->body = $icalendar;
     ...
     */
    public function SetMatch($match, $etag = '*')
    {
     ...
     */
    public function DoHEADRequest($url)
    {
        $this->body = "";
     ...
     */
    public function CalendarUrls($urls = null)
    {
        if (isset($urls)) {
            if (! is_array($urls)) {
                $urls = array($urls);
     ...
     */
    public function HrefForResourcetype($tagname, $i = 0)
    {
    ...
    public function FindCalendars($recursed = false)
    {
     ...
     */
    public function FindCalendarHome($recursed = false)
    {
     ...
     */
    public function FindPrincipal($url = null)
    {
     ...
     */
    public function DoPROPFINDRequest($url, $props, $depth = 0)
    {
        $this->SetDepth($depth);
     ...
     * @param integer $which Which instance of the tag should we use
     */
    public function HrefForProp($tagname, $i = 0)
    {
        if (isset($this->xmltags[$tagname]) && isset($this->xmltags[$tagname][$i])) {
            $j = $this->xmltags[$tagname][$i];
            while ($j-- > 0 && $this->xmlnodes[$j]['tag'] != 'DAV::href') {
     ...
     */
    public function HrefValueInside($tagname)
    {
     ...
     */
    public function CalendarHomeSet($urls = null)
    {
        if (isset($urls)) {
            if (! is_array($urls)) {
                $urls = array($urls);
     ...
     */
    public function parse_calendar_info()
    {
        $calendars = array();
     ...
     */
    public function GetOKProps($nodenum)
    {
        $props = null;
     ...
     */
    public function GetCalendarDetailsByURL($url)
    {
        $properties =
            array(
                'resourcetype',
     ...
     */
    public function GetCalendarDetails($url = null)
    {
        if (isset($url)) {
     ...
     */
    public function SetCalendar($url)
    {
     ...
     */
    public function GetCollectionETags($url = null)
    {
        if (isset($url)) {
            $this->SetCalendar($url);
        }
     ...
     */
    public function CalendarMultiget($event_hrefs, $url = null)
    {
...
EOFILTER;
     ...
     */
    public function DoCalendarQuery($filter, $url = null)
    {
...
EOXML;
     ...
     * @return array An array of the relative URLs, etags, and events, returned from DoCalendarQuery() @see DoCalendarQuery()
     */
    public function GetTodos($start = null, $finish = null, $completed = null, $cancelled = null, $relative_url = "")
    {
        $this->SetDepth('1');
        ...
        } elseif (isset($completed) && $completed == false) {
        ...
        } elseif (isset($cancelled) && $cancelled == false) {
...
EOFILTER;
     ...
     */
    public function GetEntryByUid($uid, $relative_url = null)
    {
        $this->SetDepth('1');
        $filter = "";
        if ($uid) {
...
EOFILTER;
     ...
     */
    public function DoPROPPATCH($xml_text, $url)
    {
        ...
        } elseif ($this->httpResultCode != 200) {
     ...
     */
    public function DoXMLRequest($request_method, $xml, $url = null)
    {
        $this->body = $xml;
     ...
     */
    public function principal_property_search($xml_text, $url)
    {
        $result = array();
